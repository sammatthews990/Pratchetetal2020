...
SvsG = df %>% dplyr::select(-index, -p.value, -stat) %>%
  gather(Hab, value, -Year, -Species,-Type) %>%
  spread(Year, value, fill = NA) %>%
  gather(Year, value, -Species, -Hab, -Type) %>%
...
# MAke Tile Graph
ggplot(SvsG, aes(x=Hab,y=Year, fill = as.factor(value))) + geom_tile() + 
  scale_y_discrete(limits = rev(levels(SvsG$Year))) +
  facet_wrap(~Species) + 
  scale_fill_manual(values = c("seagreen2", "grey50")) + theme_bw(base_size = 14)
...
SvsGAll = dfAll %>% dplyr::select(-index, -p.value, -stat) %>%
  gather(Hab, value, -Species,-Type) %>%
  mutate(Hab = factor(Hab, levels = c("s.Base", "s.Slope", "s.Crest", "s.Flat")),
         value = ifelse(value ==0, NA, value))

ggplot(SvsGAll, aes(x=Hab,y=Species, fill = as.factor(value))) + geom_tile() + 
  scale_fill_manual(values = c("seagreen2", "grey50")) + theme_bw(base_size = 14)
